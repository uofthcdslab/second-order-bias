...
from utils.parsing import clean_response_dict


master_df_annotated = pd.read_csv(settings.paths["input_biased_text_csv"])
...
sob_results = pd.DataFrame(rows, columns=['model_name', 'prompt_name', 'data_name', 'id', 'text', 'target_dem_val', 'target_dem_cat', 'target_type', 'target_dem_val_sec', 'target_dem_cat_sec', 'response', 'reasoning', 'gen_error'])

sob_results.to_csv(settings.paths["sob_results_csv"], index=False, encoding='utf-8')
...
mask = (
    sob_results["prompt_name"].isin(target_prompts)
    & (sob_results["gen_error"] == "generation_error")
)

sob_results.loc[mask, "response"] = sob_results.loc[mask, "response"].apply(clean_response_dict)
