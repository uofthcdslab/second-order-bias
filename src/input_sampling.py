...
data = pd.read_parquet(PROJECT_ROOT / "data/downloaded/ishate_test.parquet.gzip")
...
ishate_sampled.to_csv(PROJECT_ROOT / 'data/sampled/ishate_sampled_500.csv', index=False)
...
toxigen = pd.read_csv(PROJECT_ROOT / "data/downloaded/toxigen_annotated_test.csv")

hate_toxigen = toxigen[toxigen['toxicity_human'] > 2.5]
hate_toxigen.to_csv(PROJECT_ROOT / 'data/sampled/toxigen_sampled_500.csv', index=False)
...
hatecheck = pd.read_csv(PROJECT_ROOT / 'data/downloaded/hatecheck_test.csv')

hatecheck_hate = hatecheck[hatecheck['label_gold'] == 'hateful']
sampled_hatecheck = hatecheck_hate.sample(n=500, random_state=42)

sampled_hatecheck.to_csv(PROJECT_ROOT / 'data/sampled/hatecheck_sampled_500.csv', index=False)
...
dynab = pd.read_csv(PROJECT_ROOT / 'data/downloaded/Dynamically Generated Hate Dataset v0.2.3.csv')
...
sampled_dynab.to_csv(PROJECT_ROOT / 'data/sampled/dynab_sampled_500.csv', index=False)
print(f"DynaB saved: {sampled_dynab.shape}")


# ── LingHate ───────────────────────────────────────────────────────────────────

ling = pd.read_csv(PROJECT_ROOT / 'data/downloaded/linguistic_informed_data.csv', sep='\t')
ling.to_csv(PROJECT_ROOT / 'data/downloaded/transformed_linguistic_informed_data.csv', index=False)

sampled_ling = stratified_sample(ling, col="TARGET", n=500)

sampled_ling.to_csv(PROJECT_ROOT / 'data/sampled/linghate_sampled_500.csv', index=False)
